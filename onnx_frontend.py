...
def parse_onnx_to_prometheus(onnx_path="deit_model.onnx"):
    """
    绕过 PoCC，直接将 ONNX 算子映射为 Prometheus 语法树/节点列表
    """
    ...
    # We will compile the first MatMul node for this real test
    target_node = matmul_nodes[0]
    print(f"[ONNX Frontend] Selected target node for HLS compilation: {target_node.name}")
    ...
    prometheus_nodes.append(p_node)
    return prometheus_nodes

def mock_deit_layer():
    """没有真实 ONNX 文件时的 Mock 数据：DeiT 的注意力查询降维操作"""
    # X: [197, 768] (Seq_len=197, Dim=768) 
    # Wq: [768, 768]
    node = PrometheusNode("MatMul", "Attention_Q_Proj", 
...
if __name__ == "__main__":
    ...
    nodes = parse_onnx_to_prometheus(onnx_path)
    ...
    for n in active_nodes[:5]: # Print first 5 active nodes
        ...
        if n.loop_bounds:
            print("  [Generated Prometheus Loops]")
